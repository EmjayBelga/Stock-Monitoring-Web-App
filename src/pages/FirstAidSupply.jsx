...
const AddFirstAid = lazy(() => import('../components/Dashboard/Add/AddFirstAid'));
const EditFirstAid = lazy(() => import('../components/Dashboard/Edit/EditFirstAid'));


const FirstAidSupply = ({userInfo, isUserAdmin}) => {
  ...
  const getSupplies = () => {
    onSnapshot(collection(db, "firstAidSupplies"), (snapShot) => {
  ...
  const handleDelete = id => {
    Swal.fire({
      ...
      if (result.value) {
        ...
        deleteDoc(doc(db, "firstAidSupplies", id));
      ...
      {!isAdding && !isEditing && (
        <>
          <Header 
           ...
           />
          <h2 className="admin__sub-title">First Aid Supply Details</h2>
